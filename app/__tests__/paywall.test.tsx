...
describe("Paywall", () => {
  ...
  // This app's paywall pairs claims two-to-a-card rather than listing them
  // one row at a time. Both members of the first pair must be on screen
  // together — the carousel does not virtualize its (small) card list away.
  it("pairs the first two claims onto one card", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("feat1Title"))).toBeTruthy();
